...
const RadarViewer = ({ initialLatitude, initialLongitude }) => {
  ...
  useEffect(() => {
    ...
    if (mapContainer) {
      map = L.map(mapContainer).setView([initialLatitude, initialLongitude], 6);

      L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}', {
        attribution: 'Rampart Technologies, FOO',
        maxZoom: 18,
        id: 'mapbox/satellite-v9',
        tileSize: 512,
        zoomOffset: -1,
        accessToken: 'pk.eyJ1IjoicmFtcGFydHRlY2giLCJhIjoiY2xyOGhqNTgzMng2ejJrcGF6bjY1azVneiJ9.SVLuK_wVsEcbp7IVTUNYHA', // Replace with your Mapbox access token
      }).addTo(map);
  ...
  const isTilesLoading = () => loadingTilesCount > loadedTilesCount;
  ...
  const addLayer = (frame) => {
    if (!radarLayers[frame.path]) {
      ...
      const source = new L.TileLayer(
        ...
        {
          tileSize: 256,
          opacity: 0.5, // Adjust opacity here (e.g., 0.5 for 50% transparency)
          zIndex: frame.time,
        }
      );
